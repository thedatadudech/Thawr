...
// Copied from tailscale.com/net/stun; the SOFTWARE attribute identifies
// Thawr clients so the server answers only them.
...
const (
	...
	software       = "thawr-01" // notably: 8 bytes long, so no padding
...
// NewTxID returns a new random TxID. crypto/rand.Read cannot fail
// since Go 1.24, so the error is not surfaced.
func NewTxID() TxID {
	var tx TxID
	_, _ = crand.Read(tx[:])
	return tx
...
func appendU16(b []byte, v uint16) []byte {
	return append(b, byte(v>>8), byte(v)) //nolint:gosec // byte-wise big-endian encoding
}

func appendU32(b []byte, v uint32) []byte {
	return append(b, byte(v>>24), byte(v>>16), byte(v>>8), byte(v)) //nolint:gosec // byte-wise big-endian encoding
}

// ParseBindingRequest parses a STUN binding request.
//
// It returns an error unless it advertises that it came from
// Thawr (SOFTWARE attribute).
...
// Codec errors.
var (
	// ErrNotSTUN means the packet has no STUN header.
	ErrNotSTUN = errors.New("stun: not a STUN packet")
	// ErrNotSuccessResponse means the packet is not a binding success.
	ErrNotSuccessResponse = errors.New("stun: packet is not a success response")
	// ErrMalformedAttrs means an attribute could not be parsed.
	ErrMalformedAttrs = errors.New("stun: malformed attributes")
	// ErrNotBindingRequest means the request is not a binding request.
	ErrNotBindingRequest = errors.New("stun: not a binding request")
	// ErrWrongSoftware means the request did not come from a Thawr client.
	ErrWrongSoftware = errors.New("stun: request came from non-Thawr software")
	// ErrNoFingerprint means the request does not end in a FINGERPRINT.
	ErrNoFingerprint = errors.New("stun: request lacks a fingerprint")
	// ErrWrongFingerprint means the FINGERPRINT did not verify.
	ErrWrongFingerprint = errors.New("stun: bogus fingerprint")
)
...
func Response(txID TxID, addrPort netip.AddrPort) []byte {
	...
	b = appendU16(b, uint16(attrsLen)) //nolint:gosec // attrsLen is at most 24
	b = append(b, magicCookie...)
	b = append(b, txID[:]...)

	// Attributes (well, one)
	b = appendU16(b, attrXorMappedAddress)
	b = appendU16(b, uint16(4+addr.BitLen()/8)) //nolint:gosec // at most 20
	b = append(b,
		0, // unused byte
		fam)
	...
	for i, o := range ipa[16-addr.BitLen()/8:] {
		if i < 4 {
			...
			b = append(b, o^txID[i-len(magicCookie)]) //nolint:gosec // i < 16, cookie is 4 bytes, txID 12
		}
	}
	return b
}
...
func xorMappedAddress(tID TxID, b []byte) (addr []byte, port uint16, err error) {
	...
	for i := range xorAddr {
		if i < len(magicCookie) {
			...
			addr[i] = xorAddr[i] ^ tID[i-len(magicCookie)] //nolint:gosec // i < addrLen <= 16, cookie is 4 bytes, txID 12
		}
	}
